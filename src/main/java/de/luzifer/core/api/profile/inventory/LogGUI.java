        ...
        if (folder.listFiles() != null && Objects.requireNonNull(folder.listFiles()).length > 0) {
            ...
            if (e.getSlot() != 45 && e.getSlot() != 53) {
                if (e.getCurrentItem() != null) {
                    ...
                    if (!item.hasItemMeta() || !item.getItemMeta().hasDisplayName() || item.getType() != XMaterial.BOOK.parseMaterial())
                        return;
                    
                    InsideLogGUI insideLogGUI = new InsideLogGUI(new File(folder, e.getCurrentItem().getItemMeta().getDisplayName().replaceAll("§c", "") + ".yml"));
